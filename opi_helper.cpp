...
qint32 readUCEvents(HANDLE *comportptr)
{
    ...
    if(opiuce_status(comportptr, &ucOpipkt)) return -1;

    if(opiuce_evcapread(comportptr, &evOpipkt)) return -1;
    ...
    sysTS = (QDateTime::currentMSecsSinceEpoch() - QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
    for(i = 0; i < eventCt; i++)
    {       
    ...
    firstEventRefEpochMSecs = eventTS*1000/UCERTCFREQ;
    // Conversion to QDateTime, ref date & time for all sensors is 2012/sep/28 08:00:00.000
    ...
    outfileName.append(QString("E%1_TAG.txt").arg(eventDT.toString("yyyyMMdd_hhmmss")));
    ...
    *outf << QString("0.000\tS%1").arg(eventDT.toString("yyyyMMdd_hhmmss")) << endl;
    ...
    for(; i < eventCt; i++)
    {
        ...
        eventRefEpochMSecs = eventTS*1000/UCERTCFREQ;
        // Conversion to QDateTime, ref date & time for all sensors is 2012/sep/28 08:00:00.000
...
{
    if (opiuce_settsertc(comportptr)) return -1;
    else return 0;
}
...
qint32 readMMPkts(HANDLE *comportptr, qint32 fromPktNum, qint32 toPktNum)
{
    ...
    if(opiuce_get5mmtsdata(comportptr, fromPktNum, &mmOpipkt) == -1)   return -1;
    ...
    mmRefEpochMSecs = mmFirstFrmTS*1000/UCERTCFREQ;
    // Conversion to QDateTime, ref date & time for all sensors is 2012/sep/28 08:00:00.000
    mmFirstFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs(mmRefEpochMSecs);

    outfileName = QString("E%1_%2").arg(mmFirstFrmDT.toString("yyyyMMdd_hhmmss")).arg(tspdn);
    ...
    for(i = 1; i < 20; i++)
    {
        ...
        if(opiuce_get5mmtsdata(comportptr, pktNum+fromPktNum, &mmOpipkt) == -1)   return -1;
    ...
    {
        if(opiuce_get5mmtsdata(comportptr, i, &mmOpipkt))
        {
            ...
            if(opiuce_get5mmtsdata(comportptr, i, &mmOpipkt))
            {
                ...
                if(opiuce_get5mmtsdata(comportptr, i, &mmOpipkt))
                {
                    ...
                    if(opiuce_get5mmtsdata(comportptr, i, &mmOpipkt))
                    {
                        killTime = QTime::currentTime().addMSecs(100);
                        while(killTime > QTime::currentTime());
                        if(opiuce_get5mmtsdata(comportptr, i, &mmOpipkt))   // try 5 times before aborting
                        {
                            outfile->close();
                            delete outf;
                            delete outfile;
...
qint32 readMMPktsToEDF(HANDLE *comportptr, qint32 fromPktNum, qint32 toPktNum)
{
    ...
    outfileName = QString("E%1_%2").arg(QDateTime::fromMSecsSinceEpoch(mmFirstFrmDT.toMSecsSinceEpoch()).toString("yyyyMMdd_hhmmss")).arg(tspdn);
    ...
    {
        pktNum = (toPktNum-fromPktNum)*i/20;
        if(opiuce_get5mmtsdata(comportptr, pktNum+fromPktNum, &tmpOpipkt) == -1)   return -1;
    ...
    {        
        ...
        if((tmpOpipkt.length != (TSEFRMLEN)) && (tmpOpipkt.length != (TSEFRMLEN-4))) continue; // wrong packet size
        ...
        if(tmpOpipkt.length == (TSEFRMLEN))    // adcdata of 64 case
        {
...
//            }
            ...
            {
                frmOffset = prevFrmTS+((double) (ADCLEN*UCERTCFREQ))/((double) (TSERTCFREQ)) - frmTS;
            }
        ...
        if(adcQV.size() > ADCLEN*FRMSPERSEC*EDFDRDURSEC)
        {
            ...
            if(!totalDataRecCt)
            {
                ...
                beginOffFrms = ((double) (mmFirstFrmDT.toMSecsSinceEpoch() % 1000))*(((double) TSERTCFREQ)/1000/ADCLEN);
                ...
                {
                    tsQV.prepend(mmFirstFrmTS-(UCERTCFREQ*ADCLEN/TSERTCFREQ)*beginOffFrmsi);
                    skpQV.prepend(0);
                    batQV.prepend(1);
                    sqQV.prepend(0);
                    edQV.prepend(0);
                ...
                {
                    tsQV.prepend(mmFirstFrmTS-(UCERTCFREQ*ADCLEN/TSERTCFREQ)*(beginOffFrmsi+1));
                    skpQV.prepend(0);
                    batQV.prepend(1);
                    sqQV.prepend(0);
                    edQV.prepend(0);
                ...
                for(k = 0; k < tagTSQV.size(); k++)
                    tagTSQV.replace(k, tagTSQV.at(k)+((qint64) (beginOffFrms*(((float) UCERTCFREQ)/TSERTCFREQ*ADCLEN))));
            }
    ...
    tagTSQV.append(tsQV.last()+UCERTCFREQ/TSERTCFREQ*ADCLEN+((qint64) (beginOffFrms*(((float) UCERTCFREQ)/TSERTCFREQ*ADCLEN))));
    tagTextQV.append("DataEnd");

    // put in default values for last record
    while(tsQV.size() < 1*FRMSPERSEC*EDFDRDURSEC)
    {
        tsQV.append(tsQV.at(tsQV.size()-1)+UCERTCFREQ/TSERTCFREQ*ADCLEN); //
        skpQV.append(0);
        batQV.append(1);
        sqQV.append(0);
        edQV.append(0);
...
qint32 tagFileRead(QString tagfileName, QVector<qint64> *tagTSQVp, QVector<QString> *tagTextQVp)
{
    ...
    startTS = (QDateTime::fromString(fileNameTime,"yyyyMMdd_HHmmss").toMSecsSinceEpoch() - QDateTime::fromString("20120928080000000","yyyyMMddHHmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
    ...
    while(!tagstrp->atEnd())
    {
        ...
        tempi64 = tempi64*UCERTCFREQ/1000+startTS;
                 ...
                 QVector<qint64> *tagTSQVp, QVector<QString> *tagTextQVp)
{
    ...
    for(j = 0; j < tmpQVp->size()/(FRMSPERSEC*EDFDRDURSEC); j++)
    {
        ...
        for(i = 0; i < 2; i++)
        {
            if((tagTSQVp->size() > 0) && (tagTSQVp->at(0) <= (tsQVp->at(j*1*FRMSPERSEC*EDFDRDURSEC) + 1*FRMSPERSEC*EDFDRDURSEC*UCERTCFREQ*ADCLEN/TSERTCFREQ)))
            {
...
qint32 eraseMM(HANDLE *comportptr)
{
    ...
    if(opiuce_mmerasest(comportptr)) return -1;
    ...
    {
        qApp->processEvents();
        resMMEraseEnd = opiuce_mmeraseend(comportptr);
