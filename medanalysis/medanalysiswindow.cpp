...
//    qDebug() << "Total" << epochTotal <<"W"<< epochW << "P" << epochP << "M2"<< epochM2;
    ...
    if(doTxtFile)
    {
        ...
        outFileName.append(QString("E%1_%2_med_sum.txt").arg(datetimestr).arg(pdnDes));
        outfileTp = new QFile(outFileName);
        if(outfileTp->exists())
        {
            msgBox.setWindowTitle("Output Text File exists");
            msgBox.setText(QString("Out file %1 exists in selected directory.").arg(QString("E%1_%2_med_summary.txt").arg(datetimestr).arg(pdnDes)));
        ...
        *outTstrp << QString("<Meditate>(not for medical use)\n");
        ...
        // epoch scores
        outFileName = outDirName;
        outFileName.append(QString("E%1_%2_med.txt").arg(datetimestr).arg(pdnDes));
        outfileTp = new QFile(outFileName);
        if(outfileTp->exists())
        {
            msgBox.setWindowTitle("Output File exists");
            msgBox.setText(QString("Out file %1 exists in selected directory.").arg(QString("E%1_%2_med_epochscores.txt").arg(datetimestr).arg(pdnDes)));
            msgBox.setInformativeText("Do you want to overwrite?");
            msgBox.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
            if(msgBox.exec() == QMessageBox::Cancel)
            {
    ...
    // EDF file
    outFileName = outDirName;
    outFileName.append(QString("E%1_%2_med.edf").arg(datetimestr).arg(pdnDes));
