...
qint32 ConsoleWindow::ucRefresh(HANDLE *comportptr)
{
    ...
    nowTS = (QDateTime::currentDateTime().toMSecsSinceEpoch() - QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
    if(((nowTS-ucCurrTS) > UCERTCFREQ/10) || ((nowTS-ucCurrTS) < -UCERTCFREQ/10))
    {
    ...
    else
    {
        ...
        evRefEpochMSecs = evTS*1000/UCERTCFREQ;
    ...
    if(ucusdStatus & 0x01)
    {
        if(opiuce_tsestatus(comportptr,&tsOpipkt)) return -1;
        ...
        nowTS = (QDateTime::currentDateTime().toMSecsSinceEpoch() - QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*TSERTCFREQ/1000;

        if(((nowTS-tsCurrTS) > TSERTCFREQ/10) || ((nowTS-tsCurrTS) < -TSERTCFREQ/10))
        {
        ...
        if(tsSlotPdns[0] == tspdn)  // first line, slot A
        {
            ...
            {
                if(opiuce_settsezbchan(comportptr, uczbChan)) return -1;
            }
    ...
    if(ucusdStatus & 0x02)
    {
        ...
        if(opiuce_get5mmtsdata(comportptr,0,&mmOpipkt) == -1) return -1;
        ...
        else
        {
            ...
            memRefEpochMSecs = memTS*1000/UCERTCFREQ;
            ...
            {
                pktNum = 327680*i/20;
                if(opiuce_get5mmtsdata(comportptr, pktNum, &mmOpipkt) == -1)   return -1;
...
void ConsoleWindow::on_contRefreshPB_clicked()
{
    ...
    opiuce_onmode(&comport);    // go to on mode
    opiuce_turnmodon(&comport);    // go to on mode
...
void ConsoleWindow::on_sensShutdownPB_clicked(bool checked)
{
    ...
    {
        opiuce_offmode(&comport);   // go to off mode
        if(opiuce_turnmodoff(&comport))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
...
void ConsoleWindow::on_contShutdownPB_clicked()
{
    ...
    opiuce_shutdown(&comport);  // don't really need to look at return value, since shutdown
...
void ConsoleWindow::on_sensAUnpairPB_clicked()
{
    ...
    if(opiuce_forgettsesettings(&comport, tsSlot))    // A is always first sensor
    {
        defaultConsoleState();
        opi_closeuce_com(&comport);
        conReturnCom();
...
void ConsoleWindow::on_sensBUnpairPB_clicked()
{
    ...
    if(opiuce_forgettsesettings(&comport, tsSlot))
    {
        defaultConsoleState();
        opi_closeuce_com(&comport);
        conReturnCom();
...
qint32 ConsoleWindow::ucPair(HANDLE *comportptr)
{
            ...
            (uctsList[3] == 0xFF))
    {
        ...
        for(i = 0; i < 16; i++)
        {
            if(opiuce_setzbchan(comportptr, i+11))
            {
                return -1;
            }
        ...
        // set new zb channel
        if(opiuce_setzbchan(comportptr, uczbChan))
        {
            return -1;
        }
    }
    ...
    opiuce_settserftxpwr(comportptr, DEFRFTXPWR);

    // set the ts zbChan to the same as the uc
    if(opiuce_settsezbchan(comportptr, uczbChan)) return -2;

    // get the ts information
    if(opiuce_tsestatus(comportptr, &tsOpipkt)) return -2;
    ...
    if((uctsList[0] == tspdn) || (uctsList[1] == tspdn) || (uctsList[2] == tspdn) || (uctsList[3] == tspdn))
    {
        ...
        tspdn = tempi16;
        if(opiuce_settsepdn(comportptr, tspdn)) return -2;
    }

    // do the actual copying to uc
    if(opiuce_copytsesettings(comportptr, tsSlot)) return -1;
...
void ConsoleWindow::on_sensARFRB_clicked(bool checked)
{
    ...
    if(opiuce_tsestatus(&comport, &tsOpipkt))
    {
        defaultConsoleState();
        opi_closeuce_com(&comport);
        conReturnCom();
    ...
    if(checked)
    {
        if(opiuce_settserfmode(&comport,(tsRFMode & 0x02) | 0x01))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
    ...
    {
        if(opiuce_settserfmode(&comport,(tsRFMode & 0x02)))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
    ...
    if(opiuce_copytsesettings(&comport, tsSlot))
    {
        defaultConsoleState();
        opi_closeuce_com(&comport);
        conReturnCom();
...
void ConsoleWindow::on_sensAMMRB_clicked(bool checked)
{
    ...
    if(checked)
    {
        if(opiuce_settsemmwrite(&comport, 1))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
    ...
    {
        if(opiuce_settsemmwrite(&comport, 0))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
...
void ConsoleWindow::on_sensADTRB_clicked(bool checked)
{
    ...
    if(checked)
    {
        if(opiuce_settserfmode(&comport,(tsRFMode & 0x01) | 0x02))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
    ...
    {
        if(opiuce_settserfmode(&comport,(tsRFMode & 0x01)))
        {
            defaultConsoleState();
            opi_closeuce_com(&comport);
            conReturnCom();
