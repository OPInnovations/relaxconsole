...
{
    if((opiuce_getwltsedata(&com,&PACKAGE_tp[0])==NEWDATAGETCODE_BF)) //check new data
    {
...
int startwindowbf::opiuce_getPDN(void)
{
    int totalcount=0;
    int i;

    opiuce_status(&com,&PACKAGE_tp[0]);
...
void startwindowbf::showEvent(QShowEvent *)
{
    if((*comPortUserp==BFCOMPORTFREE||*comPortUserp==BFCOMPORTUSER)&&(!gamemode))
    {
        ...
        hide_check_pdn();
        portOn = false;
        if(opi_openuce_com(&com)==0)
        {
            portOn = true;
            *comPortUserp = BFCOMPORTUSER;
            get_opipkt_total=opiuce_getPDN();
            if(!get_opipkt_total)
            {
...
void startwindowbf::on_pushButton_2_clicked()
{
    ...
    else
    {
        ...
        opiuce_status(&com, &opipkttmp);    // should have opened device successfully before
        QDateTime stDT = QDateTime::currentDateTime();
        regulatefilename = QString("E%1_ALL").arg(stDT.toString("yyyyMMdd_hhmmss"));
        if(ui->checkWriteFileBox->isChecked()) //if true then write it
        {
            ...
            fwrite(opipkttmp.payload, 1, OPIUCESTLEN-1, file);
            for(i = 0; i < (512-(OPIUCESTLEN-1)); i++) tempui8arr[i] = 0xFF;
            fwrite(tempui8arr, 1, (512-(OPIUCESTLEN-1)), file);
        }
        else
            firstfile=true;
