...
//         qDebug()<<"reset R1";
        while(!ui->TimeSlider->isEnabled())
            qApp->processEvents();
//         qDebug()<<"reset R2";
        if(accxqvect.count()/FWSAMPLERATEACCX>FWMAXSECONDINONESCENE) //ReLax
            ui->TimeSlider->setMaximum(FWMAXSECONDINONESCENE);//
        else
            ui->TimeSlider->setMaximum(accxqvect.count()/FWSAMPLERATEACCX-1);//ReLax
    }
    else if(Filetype<7)
    {
        while(ui->TimeSlider==NULL)
...
//         qDebug()<<"reset R3";
        while(!ui->TimeSlider->isEnabled())
            qApp->processEvents();
//         qDebug()<<"reset R4";
        if(accxqvect.count()/ECGSAMPLERATE>FWECGMAXSECONDINONESCENE) //ReLax
            ui->TimeSlider->setMaximum(FWECGMAXSECONDINONESCENE);//
        else
            ui->TimeSlider->setMaximum(accxqvect.count()/ECGSAMPLERATE-1);//ReLax
    }
    else if(Filetype<9)
    {
        while(ui->TimeSlider==NULL)
...
//         qDebug()<<"reset R5";
        while(!ui->TimeSlider->isEnabled())
            qApp->processEvents();
//         qDebug()<<"reset R6";
        if(M2qvect.count()/EEGSAMPLERATE>FWEEGMAXSECONDINONESCENE)  //ReLax
           ui->TimeSlider->setMaximum(FWEEGMAXSECONDINONESCENE);//
        else
           ui->TimeSlider->setMaximum(M2qvect.count()/EEGSAMPLERATE-1);//ReLax
    }
    ...
    {
        checktimeandline(ui->drawSignalA,signal_index_begin,EEGSAMPLERATE);
        ...
        checktimeandline(ui->drawSignalA,signal_index_begin,EEGSAMPLERATE);
        if(TSqvect->count()!=0)
            showTag(ui->drawSignalF,sceneSignalF,TSqvect,Textqvect,signal_index_begin,EEGSAMPLERATE);
    }
}
    ...
    ui(new Ui::signalviewer)
{
    ...
    if(type<7)
    {
        ...
        else
        {
            ...
            {
                // make sure there is enough data for another data record, otherwise get out
                if((((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > adcqvectp->count()) ||
                        (((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > tempqvectp->count()) ||
                        (((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > accxqvectp->count()) ||
                        (((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > accyqvectp->count()) ||
                        (((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > acczqvectp->count()) ||
                        (((j+1)*ECGSAMPLERATE*EDFDRDURSEC-1) > mytsPointer->count()))
                    break;
                qApp->processEvents();
                for(i = 0; i < ECGSAMPLERATE*EDFDRDURSEC; i++)
                    adcqvect.append( (*adcqvectp)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]);//RR
                for(i = 0; i < ECGSAMPLERATE*EDFDRDURSEC; i++)
                    accxqvect.append( (*accxqvectp)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]);//amp
                for(i = 0; i < ECGSAMPLERATE*EDFDRDURSEC; i++)
                    accyqvect.append( (*accyqvectp)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]);//LFper=SDNN
                for(i = 0; i < ECGSAMPLERATE*EDFDRDURSEC; i++)
                    acczqvect.append( (*acczqvectp)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]); //HFper=HFpower log
                for(i = 0; i < ECGSAMPLERATE*EDFDRDURSEC; i++)
                    tempqvect.append( (*tempqvectp)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]); //LHRatio=log
                for(i = 0; i< ECGSAMPLERATE*EDFDRDURSEC; i++)
                    mytsQV.append( (*mytsPointer)[(j*ECGSAMPLERATE*EDFDRDURSEC+i)]);
            }
        }
    }
    TSqvect=Tsqvectp;
    ...
    if(type==7||type==8)
    {
        ...
        else
        {
            ...
            {  // make sure there is enough data for another data record, otherwise get out
                ...
                for(i = 0; i< EEGLEN*EEGSAMPLERATE*EEGEDFDRDURSEC; i++)
                    mytsQV.append( (*mytsPointer)[(j*EEGLEN*EEGSAMPLERATE*EEGEDFDRDURSEC+i)]);
            }
        }
    }
        ...
        for(j=0;j<samplerate;j++) //how many sample per second
        {
            ...
            if(nowvaluex>(ui->line->x())-everysecondincrease/2)
            {
                if(nowvaluex<(ui->line->x())+everysecondincrease/2)
                {
                    ...
                    // start and end DateTime settings
                    // put in default values for start and end time
                    firstFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs((middlevaluefortime)*1000/UCERTCFREQ);
                    // align to full second
                    //firstFrmDT.setMSecsSinceEpoch(((firstFrmDT.toMSecsSinceEpoch()+999)/1000)*1000);
...
//    qDebug()<<"reset B";
    ...
    if(Filetype<5)
    {
        TSSTEP=ADCLEN*UCERTCFREQ/TSERTCFREQ;
    ...
    {
        TSSTEP=UCERTCFREQ/ECGSAMPLERATE;
        SHOWTAGTHRESHOLD=TSSTEP/2;
    ...
    {
        TSSTEP=UCERTCFREQ/EEGSAMPLERATE;
        SHOWTAGTHRESHOLD=TSSTEP/2;
        ui->pushButton_spectrogram->hide();
...
void signalviewer::on_ShiftSlider_valueChanged(int value)
{
    ...
    {
        ui->timemorelabel_now->setText(QString("%1s").arg(signal_index_begin/EEGSAMPLERATE));
        checktimeandline(ui->drawSignalA,signal_index_begin,EEGSAMPLERATE);
        switchdraw(ui->drawSignalA,sceneSignalA,adcgainvalue,signal_index_begin,&changablemiddleA,EEGSAMPLERATE,ui->comboBoxSignalA->currentIndex());
        switchdraw(ui->drawSignalB,sceneSignalB,othergainvalue,signal_index_begin,&changablemiddleB,EEGSAMPLERATE,ui->comboBoxSignalB->currentIndex());
        switchdraw(ui->drawSignalC,sceneSignalC,othergainvalue,signal_index_begin,&changablemiddleC,EEGSAMPLERATE,ui->comboBoxSignalC->currentIndex());
...
void signalviewer::on_StartTimeButtion_clicked()
{
    ...
    // align to full second
    firstFrmDT.setMSecsSinceEpoch(((firstFrmDT.toMSecsSinceEpoch()+999)/1000)*1000);
    (*myfirstFrmTS) = (firstFrmDT.toMSecsSinceEpoch()-QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
    ui->stDTE->setDateTime(firstFrmDT);
...
void signalviewer::on_EndTimeButton_clicked()
{
    ...
    (*mylastFrmTS) = (*myfirstFrmTS) + (((*mylastFrmTS) - (*myfirstFrmTS))/(8*UCERTCFREQ))*8*UCERTCFREQ;

    lastFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs((*mylastFrmTS)*1000/UCERTCFREQ);

    // align to floor of full second
    lastFrmDT.setMSecsSinceEpoch(((lastFrmDT.toMSecsSinceEpoch())/1000)*1000);
    (*mylastFrmTS) = (lastFrmDT.toMSecsSinceEpoch()-QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
