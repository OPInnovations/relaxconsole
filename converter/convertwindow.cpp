...
bool ConvertWindow::convertedfMecgtoedf()
{
    ...
    firstFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs((tsQVpp[pdnSlot]->at(0))*1000/UCERTCFREQ);
    datetimestr = firstFrmDT.toString("yyyyMMdd_hhmmss");

    // Output file stuff
    outFileName = outDirName;
    outFileName.append(QString("E%1_%2_ECG.edf").arg(datetimestr).arg(pdnDes));
...
bool ConvertWindow::convertedfMecgtowav()
{
    ...
    // Output file stuff
    outFileName = outDirName;
    outFileName.append(QString("E%1_%2_ECG.wav").arg(datetimestr).arg(pdnDes));
    outfile1p = new QFile(outFileName);
    ...
    wavhdrwrite(out1strp, TSERTCFREQ/8, 5, RRQVpp[pdnSlot]->size(), 16); // 32Hz
    ...
    //ECG  LF=SDNN*10; HFper=log HFpower; LHRatio=log LHRatio
{
    ...
    // Initialization
    stepTS = (UCERTCFREQ/ECGSAMPLERATE);  //no record, 32Hz=128 counts
    ...
    while(!instrp->atEnd()) // read until no more data
    {
        ...
        // adc data 4096 bytes = sampsPerDRQV.at(0)*2bytes
        ...
        for(j = 0; j < 256; j++)  //256 samples per record 32Hz*8s
        {
...
bool ConvertWindow::convertedfMeegtoedf()
{
    ...
    // Output file stuff
    outFileName = outDirName;
    outFileName.append(QString("E%1_%2_EEG.edf").arg(datetimestr).arg(pdnDes));
    outfile1p = new QFile(outFileName);
...
bool ConvertWindow::convertedfMeegtowav()
{
    ...
    // Output file stuff
    outFileName = outDirName;
    outFileName.append(QString("E%1_%2_EEG.wav").arg(datetimestr).arg(pdnDes));
    outfile1p = new QFile(outFileName);
                                  ...
                                  QVector<quint8> *batQVp, QVector<quint8> *edQVp)
{
    ...
    // Initialization
    stepTS = UCERTCFREQ/EEGSAMPLERATE; //#ticks=512 per EEG sample 8Hz
    prevTS = (startDT.toMSecsSinceEpoch()-QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").toMSecsSinceEpoch())*UCERTCFREQ/1000;
    prevTS -= stepTS;   // must make it previous TS
    if(prevTS < 0) prevTS = 0;
    dataRecCt = 0;
...
qint32 ConvertWindow::whatInType(QString filename)
{
    ...
    {
        // check if generic opi/edf data
        if((numSignals == 7) && (dataRecDur == EDFDRDURSEC) &&
                (sampsPerDRQV.at(0) == EDFDRDURSEC*TSERTCFREQ) &&  //ADC
                (sampsPerDRQV.at(1) == EDFDRDURSEC*FRMSPERSEC) && (sampsPerDRQV.at(2) == EDFDRDURSEC*FRMSPERSEC) &&  //XY
                (sampsPerDRQV.at(3) == EDFDRDURSEC*FRMSPERSEC*ACCLEN) && (sampsPerDRQV.at(4) == EDFDRDURSEC*FRMSPERSEC) && //Z, TEMP
                (sampsPerDRQV.at(5) == EDFDRDURSEC*FRMSPERSEC) && ((sampsPerDRQV.at(6) == 64) || (sampsPerDRQV.at(6) == 30))) //ACT, Tag
        {
        ...
        if((numSignals == 6) && (dataRecDur == EDFDRDURSEC) &&
                (sampsPerDRQV.at(0) == 256) && //RR
                (sampsPerDRQV.at(1) == 256) && (sampsPerDRQV.at(2) == 256) && //Amp, SDNN
                (sampsPerDRQV.at(3) == 256) && (sampsPerDRQV.at(4) == 256)    //LF, LHratio
                && ((sampsPerDRQV.at(5) == 64) || (sampsPerDRQV.at(5) == 30))) //Tag
        {
...
bool ConvertWindow::convertopitowav()
{
    ...
    for(pdnSlot = 0; pdnSlot < PDNLISTLEN; pdnSlot++)
    {
        ...
        // Output file stuff
        outFileName = outDirName;
        outFileName.append(QString("E%1_%2").arg(datetimestr).arg(pdnDes).append("_1.wav"));
        outfile1p = new QFile(outFileName);
        outFileName = outDirName;
        outFileName.append(QString("E%1_%2").arg(datetimestr).arg(pdnDes).append("_2.wav"));
        ...
        // Wav header
        wavhdrwrite(out1strp, TSERTCFREQ, 1, adcQVpp[pdnSlot]->size(), 16);
        wavhdrwrite(out2strp, TSERTCFREQ/16, 5, azQVpp[pdnSlot]->size(), 16);
        ...
        // Show done
        outFileName = outDirName;
        outFileName.append(QString("E%1_%2_<1,2>").arg(datetimestr).arg(pdnDes).append(".wav"));
        ui->msgPTE->appendPlainText(QString("PDN#%1: WAV Conversion Finished, Saved in %2").arg(pdnDes).arg(outFileName));

        // Meta data processing
        if(outDataType == ECGMDATATYPE)
        {
            adcECGindQV = findHRmax(adcQVpp[pdnSlot], TSERTCFREQ, adcQVpp[pdnSlot]->size()/TSERTCFREQ, &missBeatFlagQV,true,true,&tempPavg,&tempDavg,&tempDlast);
            ...
            else
            {
                ...
                outFileNameDtoMecg.append(QString("E%1_%2_RR.txt").arg(datetimestr).arg(pdnDes));
                ...
                else
                {
                    ...
                    //32Hz
                    DtoMecgQVs(&adcECGindQV, adcQVpp[pdnSlot], &missBeatFlagQV, &ecgRRQV, &ecgampQV, 8);
                    if(writeRRfile)
                    writetoFile(outFileNameDtoMecg,&adcECGindQV,TSERTCFREQ);
                    wavhdrwrite(outm1strp, TSERTCFREQ/8, 5, ecgRRQV.size(), 16); // 32Hz
                    wavMecgwrite(outm1strp, &ecgRRQV, &ecgampQV, 8);
        ...
        else if(outDataType == EEGMDATATYPE)
        {
            ...
            else
            {
                ...
                eegthetaQV.clear();
                eegdeltaQV.clear();
                wavhdrwrite(outm1strp, 1, 10, adcQVpp[pdnSlot]->size()/TSERTCFREQ, 16); // 1Hz
                ...
                wavMeegwrite(outm1strp,&eegM2QV,&eegM1QV,
                             ...
                             &eegSigmaQV,&eegalphaQV,
                             &eegthetaQV,&eegdeltaQV,
                             adcQVpp[pdnSlot]->size()/TSERTCFREQ);
...
bool ConvertWindow::convertopitoedf()
{
    ...
    for(pdnSlot = 0; pdnSlot < PDNLISTLEN; pdnSlot++)
    {
        ...
        // Output file stuff
        outFileName = outDirName;
        outFileName.append(QString("E%1_%2.edf").arg(datetimestr).arg(pdnDes));
        outfile1p = new QFile(outFileName);
        if(outfile1p->exists())
        {
            msgBox.setWindowTitle("Out File Exists");
        ...
        // add data at beginning (first frame) to fill up to floor(second) (within 2 ms)
        // difference between floored exact second and first time in frames
        beginOffFrms = ((double) (firstFrmDT.toMSecsSinceEpoch() % 1000))*(((double) TSERTCFREQ)/1000/ADCLEN);
        beginOffFrmsi = (qint32) (beginOffFrms);
        beginOffFrmsj = beginOffFrms-((double) beginOffFrmsi);
        // put in default values for beginning in constant frame units
        firstFrmTS = tsQVpp[pdnSlot]->at(0);
        for(; beginOffFrmsi > 0; beginOffFrmsi--)
        {
            tsQVpp[pdnSlot]->prepend(firstFrmTS-(UCERTCFREQ*ADCLEN/TSERTCFREQ)*beginOffFrmsi);
            skpQVpp[pdnSlot]->prepend(0);
            batQVpp[pdnSlot]->prepend(1);
        ...
        if(beginOffFrmsj > 0.5)
        {
            tsQVpp[pdnSlot]->prepend(firstFrmTS-(UCERTCFREQ*ADCLEN/TSERTCFREQ)*(beginOffFrmsi+1));
            skpQVpp[pdnSlot]->prepend(0);
            batQVpp[pdnSlot]->prepend(1);
            sqQVpp[pdnSlot]->prepend(4);
            edQVpp[pdnSlot]->prepend(0);
        ...
        annText2QV = annTextQV;
        for(k = 0; k < annOnsetTS2QV.size(); k++)
            annOnsetTS2QV.replace(k, annOnsetTS2QV.at(k)+((qint64) (beginOffFrms*(((float) UCERTCFREQ)/TSERTCFREQ*ADCLEN))));

        // Add a "DataStart" and "DataEnd" tag
        annOnsetTS2QV.prepend(tsQVpp[pdnSlot]->at(0)+((qint64) (beginOffFrms*(((float) UCERTCFREQ)/TSERTCFREQ*ADCLEN))));
        annText2QV.prepend("DataStart");
        annOnsetTS2QV.append(tsQVpp[pdnSlot]->at(tsQVpp[pdnSlot]->size()-1)+(UCERTCFREQ*ADCLEN/TSERTCFREQ)+((qint64) (beginOffFrms*(((float) UCERTCFREQ)/TSERTCFREQ*ADCLEN))));
        annText2QV.append("DataEnd");
        ...
        {
            tsQVpp[pdnSlot]->append(tsQVpp[pdnSlot]->at(tsQVpp[pdnSlot]->size()-1)+(UCERTCFREQ*ADCLEN/TSERTCFREQ));
        ...
        if(outDataType == ECGMDATATYPE)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(writeRRfile)
                    writetoFile(outFileNameDtoMecg,&adcECGindQV,TSERTCFREQ);
                    edfMecgwrite(outm1strp, &ecgRRQV, &ecgampQV, 8, dataRecordCt, tsQVpp[pdnSlot]->at(0), &annOnsetTS2QV, &annText2QV);
                               ...
                               QVector<qint16> *sqQVp, QVector<quint8> *edQVp)
{
    ...
    while (!instrp->atEnd())
    {
        ...
        if(inpkt.length != (TSEFRMLEN)) continue; // wrong packet size
        wSubDataCode = inpkt.payload[0];
        if(wSubDataCode != 0x01) continue;   // needs to be truesense_e data (wireless or mem. mod)
    ...
    usedFrmCt = 1;
    frmOffset = 0;
    progQPDp = new QProgressDialog("Reading input file", QString(), 0, instrp->device()->size()/(TSEFRMLEN+7));
    ...
    while (!(instrp->atEnd()))
    {
        ...
        readFrmCt++;
        if(opipkt_get_stream(&inpkt, instrp)) continue; // something wrong with packet
        if((inpkt.length != (TSEFRMLEN)) && (inpkt.length != (TSEFRMLEN-4))) continue; // wrong packet size
        wSubDataCode = inpkt.payload[0];
        if(wSubDataCode != 0x01) continue;   // needs to be truesense_e data (wireless or mem. mod)
        frmTS = (((qint64) inpkt.payload[WFRMHDRLEN-1]) << 40) + (((qint64) inpkt.payload[WFRMHDRLEN-1+1]) << 32) +
                (((qint64) inpkt.payload[WFRMHDRLEN-1+2]) << 24) + ((inpkt.payload[WFRMHDRLEN-1+3]) << 16) +
        ...
        {
            frmOffset = tsQVp->at(tsQVp->size()-1)+((double) (ADCLEN*UCERTCFREQ))/((double) (TSERTCFREQ)) - frmTS;
        ...
        if(inpkt.length == (TSEFRMLEN))    // adcdata of 64 case
        {
