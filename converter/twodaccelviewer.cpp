...
void twoDaccelviewer::on_genActTextPB_clicked()
{
    ...
    if(outfileTp->exists())
    {
        ...
        msgBox.setText(QString("Out file %1 existed in selected directory.").arg(QString("E%1_%2_act.txt").arg(firstFrmDT.toString("yyyyMMdd_hhmmss")).arg(mypdn)));
    ...
    *outTstrp << QString("Activity Time: %1s\n").arg(totalcount/FRMSPERSEC);//ReLax
    if(totalcount!=0)
    {
        ...
        *outTstrp << QString("Intense: %1s %2\%").arg(intensecount/FRMSPERSEC).arg(intensecount*100/totalcount);
    }
