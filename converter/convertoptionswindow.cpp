...
void ConvertOptionsWindow::on_buttonBox_accepted()
{
    ...
    myfirstFrmTS = (ui->stDTE->dateTime().toMSecsSinceEpoch() - refTS)*UCERTCFREQ/1000;
    mylastFrmTS = (ui->endDTE->dateTime().toMSecsSinceEpoch() - refTS)*UCERTCFREQ/1000;
    ...
    *myoutDirp = ui->outDirLE->text();
    ...
    // write tag text file
    firstFrmDT = QDateTime::fromString("20120928080000000","yyyyMMddhhmmsszzz").addMSecs((mytsQVpp[pdnSlotWant]->at(0))*1000/UCERTCFREQ);
    datetimestr = firstFrmDT.toString("yyyyMMdd_hhmmss");
    if(convType<5)
        outFileName.append(QString("E%1_%2_generic_TAG.txt").arg(datetimestr).arg(mypdnListp[pdnSlotWant]));
    else if(convType<7)
        outFileName.append(QString("E%1_%2_ECG_TAG.txt").arg(datetimestr).arg(mypdnListp[pdnSlotWant]));
    else if(convType<9)
        outFileName.append(QString("E%1_%2_EEG_TAG.txt").arg(datetimestr).arg(mypdnListp[pdnSlotWant]));
